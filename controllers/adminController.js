...
export const updateVisionMission = async (req, res) => {
  try {
    ...
    const vision = typeof req.body?.vision === 'string' ? req.body.vision.trim().slice(0, 1000) : '';
    const mission = typeof req.body?.mission === 'string' ? req.body.mission.trim().slice(0, 1000) : '';
    ...
    return res.json({ success: true, message: "Vision & Mission updated", vision, mission });
  } catch (err) {
    return res.json({ success: false, message: err.message });
  }
};
